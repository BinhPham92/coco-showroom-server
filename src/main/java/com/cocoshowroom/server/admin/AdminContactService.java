...
class AdminContactService {
    ...
    public AdminPageResponse<ContactSubmissionResponse> list(String cursorStr, int limit) {
        ...
        List<ContactSubmission> rows = contactRepository.findForAdmin(
                before, lastId, PageRequest.of(0, clampedLimit + 1));

        boolean hasNext = rows.size() > clampedLimit;
        List<ContactSubmission> page = hasNext ? rows.subList(0, clampedLimit) : rows;
        ContactSubmission last = page.isEmpty() ? null : page.getLast();
        ...
        return new AdminPageResponse<>(
                page.stream().map(ContactSubmissionResponse::from).toList(),
                nextCursor,
                total
        );
    }
